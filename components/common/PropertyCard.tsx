...
export default function PropertyCard({ data }: PropertyCardProps) {
  return (
    <div className="relative rounded-xl shadow p-4 p w-full max-w-sm bg-white">
      {/* Discount Badge 
      ...
      <div className="mt-3 flex justify-between items-center text-sm">
        <div className="flex items-center gap-3 text-black">
          <div className="flex items-center gap-1.5 rounded-full  border border-gray-300 text-xs px-3 py-1 h-6">
             <Image src="/assets/icons/bed 1.png" alt="Bed" width={12} height={12} />
      {data.offers.bed}
      <Image src="/assets/icons/bathtub 1.png" alt="Shower" width={12} height={12} />
      {data.offers.shower}
      <Image src="/assets/icons/people 1.png" alt="Users" width={12} height={12} />
      {data.offers.occupants}
    </div>
          </div>
             <div className="ml-10 font-semibold text-gray-900">${data.price}/<span className="text-sm">n</span></div>
          </div>
        </div>
  );
}
